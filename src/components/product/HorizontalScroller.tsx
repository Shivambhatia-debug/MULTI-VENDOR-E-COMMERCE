...
export default function HorizontalScroller({ title, subtitle, children }: HorizontalScrollerProps) {
    ...
    return (
        <section className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm relative group">
            <div className="flex items-center justify-between mb-6 pb-2 border-b border-slate-50">
                <div>
                    ...
                    {subtitle && <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mt-0.5">{subtitle}</p>}
                </div>
                <div className="flex gap-2">
                    <button
                        onClick={() => scroll("left")}
                        className="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center text-slate-400 hover:bg-slate-50 hover:text-slate-900 transition-all shadow-sm"
                    >
                        <ChevronLeft size={16} />
                    </button>
                    <button
