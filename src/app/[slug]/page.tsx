...
export default function StoreCatchAllPage() {
    ...
    useEffect(() => {
        const fetchStoreData = async () => {
            try {
                ...
                const res = await fetch(`/api/python/public/lookup-store/${slug}`);
                if (res.ok) {
                    const data = await res.json();
                    setStoreData(data);
                } else {
                    setStoreData({ error: "Store not found" });
                }
            } catch (error) {
    ...
    if (isLoading) {
        return (
            <div className="h-screen flex flex-col items-center justify-center bg-white text-slate-900">
                <div className="animate-spin mb-4"><Globe size={48} className="text-slate-950" /></div>
                <p className="text-[10px] font-black uppercase tracking-[0.4em]">Initializing Hub...</p>
            </div>
        );
    }
    ...
    if (!storeData || storeData.error) {
        return (
            <div className="h-screen flex flex-col items-center justify-center bg-white">
                ...
                <h1 className="text-3xl font-black uppercase tracking-tighter text-slate-950 mb-2">Protocol 404</h1>
                <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-8">The requested store node does not exist or has been decommissioned.</p>
                <Link href="/" className="px-10 py-4 bg-slate-950 text-white rounded-full text-[10px] font-black uppercase tracking-widest">Return to Hub</Link>
            </div>
        );
    }
    ...
    const tickerColor = config.ticker_color || "#0f172a";
